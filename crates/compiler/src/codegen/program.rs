...
pub fn codegen_program(
    ...
    // --- Thin main: everything else lives in the runtime. On wasm we drive
    // wasm-ld directly (no clang driver glue), so the entry is named
    // `__main_argc_argv` — the symbol wasi-libc's `__main_void` bridges to;
    // native keeps the C `main`.
    let entry = match target {
        crate::Target::Wasm => "__main_argc_argv",
        crate::Target::Native => "main",
    };
